...
namespace SpacetimeDB
{
        ...
        where Tables : RemoteTablesBase
    {
        ...
        private readonly ConfiguredTaskAwaitable networkMessageProcessTask;
        ...
        protected DbConnectionBase()
        {
            ...
            networkMessageProcessTask = PreProcessMessages().ConfigureAwait(false);
        }
        ...
        private readonly Channel<UnprocessedMessage> _messageQueue =
            Channel.CreateUnbounded<UnprocessedMessage>(new UnboundedChannelOptions
            {
                SingleReader = false,
                SingleWriter = false
            });

        private readonly Channel<ProcessedMessage> _preProcessedNetworkMessages =
            Channel.CreateUnbounded<ProcessedMessage>(new UnboundedChannelOptions
            {
                SingleReader = false,
                SingleWriter = false
            });
        ...
        internal bool HasPreProcessedMessage => _preProcessedNetworkMessages.Reader.Count > 0;
        ...
        private async Task PreProcessMessages()
        {
            while (!isClosing)
            {
                try
                {
                    var message = await _messageQueue.Reader.ReadAsync(_preProcessCancellationToken);
                    var preprocessedMessage = PreProcessMessage(message);
                    await _preProcessedNetworkMessages.Writer.WriteAsync(preprocessedMessage, _preProcessCancellationToken);
                }
        ...
        internal void OnMessageReceived(byte[] bytes, DateTime timestamp) =>
            _messageQueue.Writer.TryWrite(new UnprocessedMessage { bytes = bytes, timestamp = timestamp });
        ...
        {
            webSocket.Update();
            while (_preProcessedNetworkMessages.Reader.TryRead(out var preProcessedMessage))
            {
